...
def message_html(message):
    patterns = (
        ...
        (r"(?:Búsqueda lineal: )?[Ee]valúa m en la posición (-?\d+)\.", lambda m: f"Búsqueda lineal: evalúa {math_inline('m')} en la posición {math_inline(m.group(1))}."),
        (r"(?:Búsqueda lineal: )?(-?\d+) es menor que (-?\d+); descarta la mitad izquierda\.", lambda m: f"Búsqueda lineal: {math_inline(f'{m.group(1)} < {m.group(2)}')}; descarta la mitad izquierda."),
        (r"(?:Búsqueda lineal: )?(-?\d+) es mayor que (-?\d+); descarta la mitad derecha\.", lambda m: f"Búsqueda lineal: {math_inline(f'{m.group(1)} > {m.group(2)}')}; descarta la mitad derecha."),
        ...
        (r"(?:Búsqueda lineal: )?Aplica búsqueda binaria en el rango \[(-?\d+), (-?\d+)\]\.", lambda m: f"Búsqueda lineal: aplica búsqueda binaria en el rango {range_inline(m.group(1), m.group(2))}."),
