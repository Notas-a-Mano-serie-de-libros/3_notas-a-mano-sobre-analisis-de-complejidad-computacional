...
def jump(a, x):
    ...
    prev = 0
    while prev < n and a[min(n, prev + step) - 1] < x:
        prev += step
...
def jump_ops(a, x):
    n = len(a)
    jump = max(1, math.isqrt(n))
    prev = 0
    steps = 0
    while prev < n:
        steps += 1
        if a[min(n, prev + jump) - 1] >= x:
            break
        prev += jump
...
ONE = lambda n: 1.0
LOG = lambda n: max(1.0, math.log2(max(n, 2)))
LOGLOG = lambda n: max(1.0, math.log2(math.log2(max(n, 4))))
SQRT = lambda n: max(1.0, math.sqrt(n))
LINEAR = lambda n: float(n)
SPACE = {c: ("Θ(1)", ONE) for c in ("mejor", "promedio", "peor")}
TERNARY_SPACE = {c: ("Θ(1)", ONE) for c in ("mejor", "promedio", "peor")}


def constant_space(_a, _x):
    return 8


def ternary_space(_a, _x):
    return 8
...
ALGORITHMS = {
    "secuencial": spec(
        "Búsqueda secuencial", sequential, {"mejor": ("Θ(1)", ONE), "promedio": ("Θ(n)", LINEAR), "peor": ("Θ(n)", LINEAR)}, counter=sequential_ops
    ),
    "binaria": spec("Búsqueda binaria", binary, {"mejor": ("Θ(1)", ONE), "promedio": ("Θ(log n)", LOG), "peor": ("Θ(log n)", LOG)}, binary_inputs, binary_ops),
    "interpolacion": spec(
        "Búsqueda por interpolación",
        interpolation,
        {"mejor": ("Θ(1)", ONE), "promedio": ("Θ(log log n)", LOGLOG), "peor": ("Θ(n)", LINEAR)},
    ...
    "exponencial": spec(
        "Búsqueda exponencial", exponential, {"mejor": ("Θ(1)", ONE), "promedio": ("Θ(log n)", LOG), "peor": ("Θ(log n)", LOG)}, counter=exponential_ops
    ),
    "ternaria": ExerciseAlgorithm(
        "Búsqueda ternaria",
        ternary,
        ternary_inputs,
        {"mejor": ("Θ(1)", ONE), "promedio": ("Θ(log₃ n)", LOG), "peor": ("Θ(log₃ n)", LOG)},
        TERNARY_SPACE,
        10**6,
        ternary_ops,
        ternary_space,
...
def run_app():
    run_laboratory(ALGORITHMS, "Capítulo 7")
