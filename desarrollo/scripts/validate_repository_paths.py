...
IGNORED_PARTS = {".git", ".mypy_cache", ".pytest_cache", ".ruff_cache", ".venv", "__pycache__", ".ipynb_checkpoints", ".virtual_documents"}
OBSOLETE_PATTERNS = {
...
PAGE_ASSET_MANIFEST = PROJECT_ROOT / "desarrollo/scripts/data/page_asset_sources.json"


def generated_page_asset_sources() -> dict[Path, Path]:
    """Devuelve recursos que Pages reconstruye antes de publicar."""

    if not PAGE_ASSET_MANIFEST.is_file():
        return {}
    manifest = json.loads(PAGE_ASSET_MANIFEST.read_text(encoding="utf-8"))
    return {PROJECT_ROOT / destination: PROJECT_ROOT / source for destination, source in manifest.items()}
...
def quoted_repository_paths(text: str) -> list[str]:
    ...
    return re.findall(
        r"[\"']((?:simulaciones/capitulo[2-8]|common|core|desarrollo/scripts)/[^\"']+)[\"']",
        text,
    )
...
def local_target_error(source: Path, target: str) -> str | None:
    ...
    if destination.exists():
        return None
    generated_assets = generated_page_asset_sources()
    generated_source = generated_assets.get(destination)
    if generated_source and generated_source.is_file():
        return None
    ...
    try:
        ...
        if source.suffix == ".md":
            ...
            if any(candidate.exists() for candidate in candidates):
                return None
            if any(generated_assets.get(candidate, Path()).is_file() for candidate in candidates):
                return None
