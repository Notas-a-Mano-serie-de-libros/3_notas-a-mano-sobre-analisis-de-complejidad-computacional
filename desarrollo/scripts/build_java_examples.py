...
def build():
    ...
    for listings in catalog["pages"].values():
        for listing in listings:
            source, inputs, name = java_example(listing, runtime=True)
            programs[name] = source
            metadata[name] = listing
    with tempfile.TemporaryDirectory(prefix="pages-java-") as folder:
        work = Path(folder)
        for name, source in programs.items():
            destination = work / Path(*name.split(".")).with_suffix(".java")
            destination.parent.mkdir(parents=True, exist_ok=True)
            destination.write_text(source)
        sources = [*work.rglob("Main.java"), *(ROOT / "desarrollo/scripts/java/ejemplos").glob("*.java")]
        ...
        for name, listing in metadata.items():
            ...
            source, entries, _ = java_example(listing, runtime=True)
            if "public static boolean buscar(int[]" in source:
                ...
                values = [literal for _, literal, _, _ in entries]
                values[0] = "{3, 1, 3}" if listing["folio"] == 363 else "{3, -1, 3}"
                expected = "[1, 3, 3]" if listing["folio"] == 363 else "[-1, 3, 3]"
                assert "Arreglo ordenado: " + expected in execute(name, "\n".join(values)), name
