...
def notebook_issues(notebook: dict) -> list[str]:
    ...
    for index, cell in enumerate(notebook.get("cells", []), start=1):
        if not "".join(cell.get("source", [])).strip() and not cell.get("outputs"):
            issues.append(f"celda {index}: vacía")
...
def clean_notebook_data(notebook: dict) -> bool:
    ...
    notebook["cells"] = [
        ...
        if "".join(cell.get("source", [])).strip() or cell.get("outputs")
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Limpia metadatos transitorios y celdas vacías de notebooks.")
