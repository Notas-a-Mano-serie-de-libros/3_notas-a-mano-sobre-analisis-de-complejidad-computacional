#!/usr/bin/env python3
"""Comprueba que todas las páginas de contenido tengan miga superior."""
...
def validate(site_dir: Path) -> list[str]:
    ...
    for page in sorted(site_dir.rglob("*.html")):
        ...
        if count != 1:
            errors.append(f"{relative}: se esperaba 1 miga de pan superior y se encontraron {count}")
    return errors
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("Migas de pan superiores validadas en todo el sitio.")
    return 0
