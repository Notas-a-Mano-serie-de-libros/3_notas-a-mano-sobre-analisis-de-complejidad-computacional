...
SIMULATION_GRID_COUNTS = {
    "simulaciones/capitulo2/runtime/experimental_animation.py": 1,
    "simulaciones/capitulo2/runtime/polynomial_animation.py": 1,
    "simulaciones/capitulo4/runtime/experimental_analysis.py": 3,
    "common/exercise_laboratory.py": 1,
...
def test_all_matplotlib_grids_match_comparacion_big_o():
    ...
    for path, source in _chart_sources():
        calls.extend((path.relative_to(PROJECT_ROOT), match.group(1).strip()) for match in GRID_CALL.finditer(source))
...
def test_all_cartesian_simulations_enable_the_reference_grid():
    ...
    shared_style = (PROJECT_ROOT / "simulaciones/capitulo2/runtime/experimental_animation.py").read_text(encoding="utf-8")
