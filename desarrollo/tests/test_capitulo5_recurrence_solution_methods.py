...
def test_tree_method_shows_tree_level_cost_and_total_sum():
    ...
    assert 'data-cost-latex="n"' in markup
    assert r'data-cost-latex="\dfrac{n}{2}"' in markup
...
def test_method_explanation_has_no_internal_horizontal_scroll():
    ...
    assert 'mjx-container[display="true"]{font-size:16px!important' in SOLUTION_STYLES


def test_method_explanation_display_equations_are_centered():
    # Las ecuaciones en modo bloque (display) deben quedar centradas, no pegadas
    # al borde izquierdo: el SVG que MathJax inserta es de ancho intrínseco y
    # display:block, así que necesita margin:auto explícito para centrarse.
    assert (
        "mjx-container>svg{display:block;max-width:100%!important;height:auto;\n"
