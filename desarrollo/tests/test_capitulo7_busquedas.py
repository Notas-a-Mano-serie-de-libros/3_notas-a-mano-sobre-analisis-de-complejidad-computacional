...
LAUNCHERS_PATH = PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "launchers.py"
LOCAL_APP_PATH = PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "abrir_busqueda.py"
...
class TestInterpolacionVisualDinamica(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.source = (
    ...
    def test_valores_y_formula_se_componen_sin_reemplazo_sin_formato(self):
        self.assertIn("function renderDynamicMath(id,latex,display)", self.source)
        self.assertIn("mathJax.tex2svgPromise(requested.latex", self.source)
        self.assertIn("if(state.running)return;", self.source)
        self.assertIn("if(rendered && node && state.pending===null)", self.source)
        self.assertIn("node.replaceChildren(rendered);", self.source)
        self.assertNotIn("vals.innerHTML='y = '", self.source)

    def test_todos_los_valores_dinamicos_utilizan_mathjax_localizado(self):
        for target in (
            "iv-v0",
            "iv-vx",
            "iv-v1",
            ...
            with self.subTest(target=target):
                self.assertIn(f"renderDynamicMath('{target}'", self.source)
        self.assertIn("'iv-fml-vals',", self.source)
        self.assertIn("dfrac", self.source)

    def test_formula_anterior_permanece_hasta_que_la_nueva_esta_lista(self):
        self.assertIn("state.pending={latex:latex,display:!!display};", self.source)
        self.assertIn("state.running=true;", self.source)
        self.assertIn("state.running=false;", self.source)
        self.assertIn(
            "if(state.pending!==null)renderDynamicMath(id,state.pending.latex,state.pending.display);",
            self.source,
        )

    def test_no_hay_ids_duplicados_para_el_valor_x(self):
        self.assertEqual(self.source.count('id="iv-dx"'), 1)
...
class TestCapitulo7BusquedaTernaria(unittest.TestCase):
    ...
    def test_bootstrap_clears_previous_widget_output(self):
        source = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8")
    ...
    def test_bootstrap_resolves_only_chapter7_launcher(self):
        ...
        self.assertIn("No se pudo localizar simulaciones/capitulo7/runtime/launchers.py", source)
        self.assertIn("project_root", source)
        self.assertIn("launcher_path.parents[3].resolve()", source)
    ...
    def test_original_ui_features_are_present(self):
        ...
        self.assertIn("compact_labeled_control", common_source)
        self.assertIn("COMPACT_GROUP_WIDTH", common_source)
        self.assertIn("target_readout = bounded_int_control(", common_source)
        ...
        self.assertIn('"common/widget_controls.py"', (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8"))
        ...
        self.assertIn('ui_state["first_row"].children = tuple(row_controls)', common_source)
        self.assertIn('display="flex"', common_source)
        self.assertIn('flex_flow="row wrap"', common_source)
        ...
        self.assertIn("render_cache.update_outputs(", common_source)
        self.assertIn("build_search_trace(state, step_search)", common_source)
        self.assertIn("for snapshot in build_search_trace(state, step_search):", common_source)
    ...
    def test_search_trace_is_lazy(self):
        common = self.launchers._load_module("search_common.py", "capitulo7_search_common_lazy_trace_test")
        state = self.module.create_state(size=8, target=8, values=[1, 2, 3, 4, 5, 6, 7, 8])
        trace = common.build_search_trace(state, self.module.step_ternary_search)

        self.assertFalse(isinstance(trace, list))
        first = next(trace)
        self.assertFalse(state["search_active"])
        self.assertTrue(first["search_active"])
    ...
    def test_individual_notebooks_keep_only_minimal_invocation_code(self):
        ...
        for notebook_name, simulation_name in expected.items():
            with self.subTest(notebook=notebook_name):
                notebook = json.loads((PROJECT_ROOT / "simulaciones" / "capitulo7" / "notebooks" / notebook_name).read_text())
                ...
                self.assertIn("BOOTSTRAP_CANDIDATES = tuple(", source)
                self.assertIn("for base in (Path.cwd(), *Path.cwd().parents)", source)
...
class TestCapitulo7BusquedaExponencial(unittest.TestCase):
    ...
    def test_exponential_ui_features_are_present(self):
        ...
        common_source = (PROJECT_ROOT / "core" / "search" / "search_common.py").read_text()
        bootstrap = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text()
        notebook = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "notebooks" / "5_busqueda_exponencial.ipynb").read_text()
        ...
        self.assertNotIn("widgets.HTMLMath", common_source)
        self.assertIn("render_cache.update_outputs(", common_source)
        self.assertIn("def build_search_trace(state, step_search):", common_source)
        self.assertIn("yield copy_search_state(probe)", common_source)
        self.assertIn("formula_output", common_source)
        self.assertIn("LABEL_HTML", source)
        self.assertIn("math_inline", common_source)
        self.assertIn("margin-top: 8px;", common_source)
...
class TestCapitulo7BusquedasRestantes(unittest.TestCase):
    ...
    def test_launchers_and_bootstrap_include_all_remaining_searches(self):
        bootstrap = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8")
    ...
    def test_notebooks_are_clean_invocations(self):
        comparison_notebook = PROJECT_ROOT / "simulaciones" / "capitulo7" / "notebooks" / "0_comparacion_busquedas.ipynb"
        comparison_nb = json.loads(comparison_notebook.read_text(encoding="utf-8"))
        comparison_cell = next(cell for cell in comparison_nb["cells"] if cell["cell_type"] == "code")
        comparison_source = "".join(comparison_cell["source"])
        self.assertIn('SIMULATION_NAME = "comparacion"', comparison_source)
        self.assertEqual(comparison_cell["outputs"], [])
        self.assertIsNone(comparison_cell["execution_count"])
        self.assertTrue(comparison_cell["metadata"]["jupyter"]["source_hidden"])

        for index, name in enumerate(("secuencial", "binaria", "interpolacion", "saltos"), start=1):
            with self.subTest(name=name):
                notebook = PROJECT_ROOT / "simulaciones" / "capitulo7" / "notebooks" / f"{index}_busqueda_{name}.ipynb"
    ...
    def test_search_apps_use_common_runner(self):
        ...
        self.assertIn('"Generar arreglo del libro"', view_source)
        self.assertIn('"core/search/search_common.py"', (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8"))
    ...
    def test_search_nodes_reserve_compact_label_height(self):
        ...
        self.assertIn(".search-array-line .search-nodes", common_source)
        self.assertIn("margin: 0;", common_source)
        self.assertIn("height: 54px;", common_source)
        self.assertIn("align-self: flex-start;", common_source)
        self.assertIn("margin-top: 38px;", common_source)
    ...
    def test_comparison_runs_all_searches_on_same_array(self):
        ...
        notebook_source = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "notebooks" / "0_comparacion_busquedas.ipynb").read_text(encoding="utf-8")
        values = [1, 2, 3, 4, 5, 6, 7, 8]
        ...
        self.assertIn("while not all_searches_complete(probe):", source)
        self.assertIn("trace = build_comparison_trace(state)", source)
        ...
        self.assertIn("grid-template-columns: minmax(0, 432px) 34px;", html)
        self.assertIn("grid-column: 1;", html)
        ...
        self.assertIn("grid-template-columns: minmax(170px, 232px) 88px minmax(0, 1fr);", source)
        self.assertIn("min-width: 0;", source)
        self.assertNotIn("overflow-x: auto;", source)
        self.assertIn("comparison-array-result-wrap", source)
        self.assertIn("comparison-array-head-label", source)
        self.assertIn("gap: 4px;", source)
        self.assertIn("gap: 0;", source)
        self.assertIn("padding-bottom: 12px;", source)
        self.assertIn("margin-bottom: 4px;", source)
        ...
        self.assertIn("grid-template-columns: minmax(170px, 232px) 88px minmax(0, 1fr);", html)
        self.assertIn("grid-template-columns: minmax(0, 432px) 34px;", html)
        self.assertIn("grid-template-columns: repeat(8, minmax(0, 54px));", html)
    ...
    def test_comparison_target_mode_controls_membership(self):
        ...
        self.assertIn("COMPACT_GROUP_WIDTH", source)
        self.assertIn("target_readout = bounded_int_control(", source)
        self.assertIn("disabled=True", source)
        self.assertIn("def first_row_controls():", source)
        self.assertIn('control_groups["target_mode"]', source)
        self.assertIn('control_groups["target_position"]', source)
        self.assertIn('first_row_box = widgets.Box(', source)
        ...
        self.assertIn('display_value = None if target_mode_input.value == TARGET_EXISTS else "none"', source)
        self.assertIn("target_position_input.layout.display = display_value", source)
        self.assertIn('control_groups["target_position"].layout.display = display_value', source)
    ...
    def test_ternary_space_complexity_is_logarithmic_in_pages(self):
        ...
        average = r"| Caso promedio | \(\Theta(\log_3(n))\) | \(\Theta(\log_3(n))\) |"
        worst = r"| Peor caso | \(O(\log_3(n))\) | \(O(\log_3(n))\) |"
        self.assertIn(average, ternary_source)
        self.assertIn(worst, ternary_source)
        self.assertIn(r"<tr><td>Caso promedio</td><td>\(\Theta(\log_3(n))\)</td><td>\(\Theta(\log_3(n))\)</td></tr>", exercises_source)
        self.assertIn(r"<tr><td>Peor caso</td><td>\(O(\log_3(n))\)</td><td>\(O(\log_3(n))\)</td></tr>", exercises_source)
