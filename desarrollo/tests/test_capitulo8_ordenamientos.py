...
CHAPTER_DIR = PROJECT_ROOT / "simulaciones" / "capitulo8"
...
class TestCapitulo8Ordenamientos(unittest.TestCase):
    ...
    def test_default_view_is_bars_and_controls_have_tree_where_needed(self):
        ...
        self.assertIn("sort_action_button_row", source)
        for name, module in self.modules.items():
            with self.subTest(name=name):
    ...
    def test_render_supports_boxes_and_bars_without_order_label(self):
        ...
        self.assertIn("color: #111827;", bar_html)
        self.assertIn("text-shadow: none;", bar_html)
    ...
    def test_merge_tree_view_follows_active_division_path(self):
        ...
        self.assertIn("#e5e7eb", second_html)
        self.assertIn("color:#6b7280", second_html)
    ...
    def test_bar_view_preserves_original_visual_style(self):
        ...
        bar_position = html.index('<div class="bar bar-role-', value_position + 1)
    ...
    def test_notebooks_are_clean_invocations(self):
        ...
        for index, name in enumerate(("burbuja", "seleccion", "insercion", "shell", "mezcla", "rapido", "radix"), start=1):
            with self.subTest(name=name):
                ...
                nb = json.loads(notebook.read_text(encoding="utf-8"))
                simulation_cell = next(cell for cell in nb["cells"] if cell["cell_type"] == "code")
                source = "".join(simulation_cell["source"])
                self.assertIn(f'SIMULATION_NAME = "{name}"', source)
                self.assertEqual(simulation_cell["outputs"], [])
                self.assertIsNone(simulation_cell["execution_count"])
                self.assertTrue(simulation_cell["metadata"]["jupyter"]["source_hidden"])
    ...
    def test_comparison_bootstrap_and_render_bars(self):
        ...
        self.assertIn('role="img" aria-label="Ordenado"', source)
        self.assertIn('aria-live="polite"', html)
        self.assertIn("contain: layout paint;", html)
        ...
        self.assertIn("render_cached_comparison_row(item, show_indexes=True)", source)
        ...
        self.assertEqual(
            html.count('<div class="comparison-index">'),
            len(values) * len(state["algorithms"]),
        )
        ...
        self.assertNotIn('description="h"', source)
        self.assertIn("primary_controls = widgets.VBox", source)
        self.assertIn("ALGORITHM_LABEL_WIDTH = 150", source)
        self.assertIn("label_width=ALGORITHM_LABEL_WIDTH", source)
        self.assertIn("ALGORITHM_FIELD_WIDTH = sum(ALGORITHM_COLUMN_WIDTHS)", source)
        self.assertIn('flex_flow="row wrap"', source)
        self.assertIn("overflow=\"visible\"", source)
        self.assertIn('grid_gap=f"{STANDARD_CONTROL_ROW_GAP}px"', source)
        self.assertIn('border_top="1px solid #767676"', source)
        self.assertIn('border_right="1px solid #767676"', source)
        self.assertIn('border_bottom="1px solid #767676"', source)
        self.assertIn('border_left="1px solid #767676"', source)
        self.assertIn('add_class("sort-algorithm-options-frame")', source)
