...
class TestPerformanceContracts(unittest.TestCase):
    def test_benchmark_script_runs_in_quick_mode(self):
        script = PROJECT_ROOT / "desarrollo" / "scripts" / "benchmark_animations.py"
    ...
    def test_benchmark_supports_thresholds_and_json_output_file(self):
        module = load_module_from_path(
            ...
            PROJECT_ROOT / "desarrollo" / "scripts" / "benchmark_animations.py",
        )
        payload = {
            "benchmarks": [
    ...
    def test_benchmark_report_uses_versioned_baseline(self):
        module = load_module_from_path(
            ...
            PROJECT_ROOT / "desarrollo" / "scripts" / "validate_benchmark_report.py",
        )
        report = {
            "benchmarks": [
                {"label": "a", "avg_ms": 2.0},
    ...
    def test_clean_notebooks_check_mode_passes_on_clean_tree(self):
        script = PROJECT_ROOT / "desarrollo" / "scripts" / "clean_notebooks.py"
        result = subprocess.run(
    ...
    def test_generated_graphics_policy_is_installed_and_clean(self):
        script = PROJECT_ROOT / "desarrollo" / "scripts" / "clean_generated_graphics.py"
        result = subprocess.run(
            [sys.executable, str(script), "--check"],
            check=True,
            capture_output=True,
        ...
        pre_push = (PROJECT_ROOT / "desarrollo" / "scripts" / "git_hooks" / "pre-push").read_text(encoding="utf-8")
        installer = (PROJECT_ROOT / "desarrollo" / "scripts" / "install_git_hooks.py").read_text(encoding="utf-8")
    ...
    def test_colab_validation_scripts_pass(self):
        for script_name in (
            ...
            with self.subTest(script=script_name):
                result = subprocess.run(
                    [sys.executable, str(PROJECT_ROOT / "desarrollo" / "scripts" / script_name)],
                    check=True,
                    capture_output=True,
                    text=True,
                    cwd=PROJECT_ROOT,
    ...
    def test_ci_workflow_has_robust_update_checks(self):
        ...
        self.assertNotIn("python desarrollo/scripts/clean_notebooks.py --check --diagnose", workflow)
        ...
        self.assertIn("python desarrollo/scripts/benchmark_animations.py --repeats 3 --max-ms 1500", workflow)
        self.assertIn("python desarrollo/scripts/validate_benchmark_report.py --report artifacts/animation-benchmark.json", workflow)
        ...
        self.assertTrue((PROJECT_ROOT / "assets" / "badges" / "ci.svg").is_file())
    ...
    def test_visual_contracts_keep_stable_dimensions_and_separated_css(self):
        search_common = (PROJECT_ROOT / "core" / "search" / "search_common.py").read_text(encoding="utf-8")
        ...
        cap7_bootstrap = (PROJECT_ROOT / "simulaciones" / "capitulo7" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8")
        cap8_bootstrap = (PROJECT_ROOT / "simulaciones" / "capitulo8" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8")
        ...
        self.assertIn("build_simulation_view(", search_common)
        self.assertIn("build_simulation_view(", sort_common)
