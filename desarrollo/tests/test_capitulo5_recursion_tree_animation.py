...
def test_bootstrap_loads_the_shared_widget_engine_in_local_and_colab_runs():
    bootstrap = (
        Path(__file__).resolve().parents[2] / "simulaciones" / "capitulo5" / "runtime" / "colab_bootstrap.py"
    ).read_text(encoding="utf-8")

    assert "sys.path.insert(0, project_root)" in bootstrap
    assert 'common/widget_controls.py' in bootstrap
    assert "simulaciones/capitulo5/runtime/recurrence_solution_methods.py" in bootstrap
...
def test_methods_notebook_uses_runtime_bootstrap_in_builder_only_mode():
    ...
    assert 'base / "simulaciones" / "capitulo5" / "runtime" / "colab_bootstrap.py"' in source
    assert 'RECURSION_TREE_RUN_KWARGS = {"builder_only": True}' in source
    assert "main/simulaciones/capitulo5/runtime/colab_bootstrap.py" in source
...
def test_general_expression_always_keeps_symbolic_external_cost():
    from simulaciones.capitulo5.runtime.recursion_tree_animation import _equation_markup
...
def test_result_expression_formats_exact_division_factors_as_fractions():
    from simulaciones.capitulo5.runtime.recursion_tree_animation import _expanded_equation_markup
...
def test_colab_tree_solver_imports_the_downloaded_tree_module():
    import subprocess
    import sys
    import textwrap
    ...
    script = textwrap.dedent('''
        ...
        repo = Path.cwd()
        bootstrap = repo / 'simulaciones/capitulo5/runtime/colab_bootstrap.py'
        class Response:
            def __init__(self, path): self.path = path
            def read(self): return self.path.read_bytes()
        def download(request, **kwargs):
            return Response(repo / request.full_url.split('/main/', 1)[1].split('?', 1)[0])
        urllib.request.urlopen = download
        with tempfile.TemporaryDirectory() as tmp:
            tempfile.gettempdir = lambda: tmp
            source = bootstrap.read_text().replace('module_path = _local_module()', 'module_path = None')
            namespace = {'__file__': str(bootstrap), 'RECURSION_TREE_RUN_KWARGS': {'builder_only': True}}
            exec(compile(source, str(bootstrap), 'exec'), namespace)
            from simulaciones.capitulo5.runtime.recursion_tree_animation import _render_svg
            from simulaciones.capitulo5.runtime.recurrence_solution_methods import solve_selected_method
            assert namespace['module']._render_svg is _render_svg
            for relation, reduction in [('division', .5), ('reduction', 1)]:
                result = solve_selected_method(relation, (2,), (reduction,), 'linear', 1, 'tree', 'standard')
                assert '<svg' in result
            print('Resolución por árbol verificada en el runtime temporal de Colab.')
    ''')
    ...
    assert "Resolución por árbol verificada" in result.stdout
