...
UI_SOURCE = (ROOT / "simulaciones" / "capitulo4" / "runtime" / "experiment_ui.py").read_text(encoding="utf-8")
C2_UI_SOURCE = (
    ROOT
    / "simulaciones" / "capitulo2"
    / "runtime"
...
def test_colab_bootstrap_reloads_the_active_common_runtime():
    source = (ROOT / "simulaciones" / "capitulo4" / "runtime" / "colab_bootstrap.py").read_text(encoding="utf-8")
...
def test_no_restringir_elimina_el_limite_en_lugar_de_cambiarlo_por_otro():
    from simulaciones.capitulo4.runtime.experiment_ui import effective_max_safe_elements
...
def test_tabla_es_nativa_y_la_medicion_tiene_rutas_local_y_colab():
    ...
    assert "def run_experiment_sync():" in UI_SOURCE
    assert "async def run_experiment_async():" in UI_SOURCE
    assert "if colab_output is not None:" in UI_SOURCE
    assert "run_experiment_sync()" in UI_SOURCE
    assert "schedule_task(run_experiment_async())" in UI_SOURCE
...
def test_plantilla_de_grafica_conserva_titulos_y_contraste():
    analysis_source = (ROOT / "simulaciones" / "capitulo4" / "runtime" / "experimental_analysis.py").read_text(
        encoding="utf-8"
    )
...
def test_ejemplos_usan_una_sola_simulacion_con_selector_temporal_espacial():
    notebooks = sorted((ROOT / "simulaciones" / "capitulo4" / "notebooks").glob("ejemplo*.ipynb"))
...
def test_remote_ui_accepts_a_shallow_temporary_module_path(monkeypatch):
    import sys

    monkeypatch.setattr(sys, "path", list(sys.path))
    namespace = {"__file__": "/tmp/chapter4/experiment_ui.py"}
    initialization = UI_SOURCE.split("try:\n    from common.experimental_simulation", 1)[0]
    exec(compile(initialization, "experiment_ui.py", "exec"), namespace)
    assert namespace["PROJECT_ROOT"].is_absolute()
