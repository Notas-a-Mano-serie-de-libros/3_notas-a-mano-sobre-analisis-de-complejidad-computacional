...
CHAPTER_DIR = Path(__file__).parents[2] / "simulaciones" / "capitulo2"
...
from experimental_animation import (
...
def test_perfiles_generales_usan_estilo_visual_de_constante():
    ...
    assert "style_experiment_axis(" in constant_source
    assert "style_experiment_axis(" in general_source
    assert "ax1.grid(True)" in theoretical_source
...
def test_comparacion_teorica_representa_la_complejidad_constante_como_c():
    notebook_path = CHAPTER_DIR.parents[1] / "recursos" / "graficas" / "capitulo2" / "comparacion_complejidades_teoricas.ipynb"
...
def test_notebooks_generales_invocan_perfiles_interactivos():
    ...
    for notebook_name, simulation_name in expected.items():
        ...
        assert 'base / "simulaciones" / "capitulo2" / "runtime"' in source
...
def test_notebooks_incluyen_grafica_teorica_correcta():
    ...
    for notebook_name, simulation_name in expected.items():
        ...
        assert 'THEORETICAL_GRAPH = "plot_theoretical_growth"' in graph_cells[0]
        assert f'THEORETICAL_ARGS = ("{simulation_name}",)' in graph_cells[0]
        assert "colab_bootstrap.py" in graph_cells[0]
        assert "urllib.request.urlopen" in graph_cells[0]


def test_notebook_logaritmico_explica_cambio_de_base():
    source = (
        CHAPTER_DIR.parents[1] / "docs" / "laboratorios" / "capitulo-2"
...
def test_referencias_colab_del_capitulo_2_siguen_orden_actual():
    abrir_source = Path(Path(__file__).parents[2] / "abrir.py").read_text(encoding="utf-8")
    readme_source = (CHAPTER_DIR.parents[1] / "README.md").read_text(encoding="utf-8")
    ...
    for path in expected_paths:
        assert (CHAPTER_DIR / "notebooks" / path).is_file()

    assert '"2/polinomial-general"' in abrir_source
    assert "7_complejidad_polinomial_general.ipynb" in abrir_source
    assert "notebooks/7_complejidad_exponencial.ipynb" not in readme_source + abrir_source
    assert "notebooks/8_complejidad_factorial.ipynb" not in readme_source + abrir_source
