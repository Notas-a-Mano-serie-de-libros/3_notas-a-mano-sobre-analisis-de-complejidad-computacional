...
class TestPerformanceContracts(unittest.TestCase):
    ...
    def test_runtime_uses_original_synchronous_pause_for_colab(self):
        ...
        self.assertIn('from IPython.display import Math, display', source)
        self.assertIn("display(Math(formula))", source)
        self.assertIn('widget.value = f"$${formula}$$" if formula else ""', source)
