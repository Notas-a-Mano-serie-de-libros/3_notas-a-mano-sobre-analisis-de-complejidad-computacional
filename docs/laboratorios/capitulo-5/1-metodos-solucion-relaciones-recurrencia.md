...
| **Teorema maestro** | Determina el crecimiento asintótico comparando \(f(n)\) con el costo crítico de las llamadas recursivas. La versión básica trata costos polinómicos, la extendida incorpora factores logarítmicos y la generalizada admite diferentes tamaños de subproblema. | Exclusivamente relaciones de **división**. Las versiones básica y extendida exigen la forma \(aC(n/b)+f(n)\) y por ello fijan \(m=1\). La versión generalizada habilita \(m>1\) porque admite varios términos \(a_iC(b_i n)\). |
