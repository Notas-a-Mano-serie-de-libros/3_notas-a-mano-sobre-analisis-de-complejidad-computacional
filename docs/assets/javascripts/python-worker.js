...
self.onmessage = async ({ data }) => {
  ...
  try {
    ...
    self.postMessage({ job: data.job, type: "ready" });
    ...
    self.postMessage({ job: data.job, type: "done", output: output || "Ejecutado sin salida.", error: false });
  } catch (error) {
    self.postMessage({ job: data.job, type: "done", output: output + "\n" + error.message, error: true });
  } finally {
    globals?.destroy();
  }
};
