...
def results_table(sizes, experimental, profile, pending=False, statuses=None):
    ...
    rows = []
    for row_index, (n, measured) in enumerate(zip(sizes, experimental, strict=True)):
...
def run_app(profile, display_app=True, mode_selector=None):
    ...
    def complete_experiment(context):
        ...
        statuses = [
            ...
            for status, value in zip(context["statuses"], context["checkpoint_times"], strict=True)
        ]
...
def run_selectable_app(profile_factory, initial_mode="time"):
    ...
    wrapper = widgets.VBox(
        [body],
        layout=widgets.Layout(width="100%", max_width="100%", grid_gap="0px"),
    )
