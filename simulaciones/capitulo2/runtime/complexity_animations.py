...
from simulaciones.capitulo2.runtime.experimental_animation import (
    ExperimentProfile,
...
T0_SECONDS = 1e-6
GRAPHICS_DIR = graphics_dir("capitulo2", "experimental")
...
def render_profile_template(config, profile, label, maximum_n):
    fig_main, ax1 = plt.subplots(1, 1, figsize=(8, 4))
...
def make_profile(name, mode="time"):
    ...
    profile = ExperimentProfile(
        mode=mode,
        function_latex=config["function_latex"],
    ...
    return replace(
        profile,
        render_template=lambda maximum_n: render_profile_template(
            config,
            profile,
            label,
            maximum_n,
...
def run_app(name, mode="time"):
    if mode in {"selectable", "both"}:
        return run_selectable_app(lambda selected_mode: make_profile(name, mode=selected_mode))
    return run_profile_app(make_profile(name, mode=mode))
