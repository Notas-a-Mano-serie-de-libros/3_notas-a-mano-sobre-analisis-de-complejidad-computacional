...
def _expanded_term_values(coefficients, values):
    expanded = []
    for coefficient, value in zip(coefficients or (), values or (), strict=True):
...
def _recurrence_text(
    ...
    terms = ", ".join(
        ...
        for coefficient, value in zip(term_a or (), term_b or (), strict=True)
    )
...
def _render_svg(
    ...
    for node in nodes:
        ...
        # Se usa siempre un <svg> nativo clonado del glifo de MathJax
        # (en vez de un <foreignObject><div>...) porque el contenido HTML de
        # un foreignObject puede quedar desalineado del sistema de
        # coordenadas del SVG tras un reflow/reescalado posterior en
        # JupyterLab (Colab no sufre este problema porque nunca lo usó).
        equation_markup = (
...
def _expanded_equation_markup(
    ...
    terms = []
    for coefficient, value in zip(term_a, term_b, strict=True):
...
def run_app(builder_only=False):
    ...
    term_validation = widgets.HTML(layout=widgets.Layout(width="252px"))
    ...
    equation = widgets.HTML(layout=widgets.Layout(width="100%"))
    expanded_equation = widgets.HTML(layout=widgets.Layout(width="100%"))
    method_solution = widgets.HTML(layout=widgets.Layout(width="100%"))
    method_solution.add_class("recursion-panel-output")
    method_solution.layout.display = "none"
    level_table = widgets.HTML(layout=widgets.Layout(width="100%"))
    figure = widgets.HTML(layout=widgets.Layout(width="100%"))
    ...
    styles = widgets.HTML(
        ...
        <style>
          ...
          .recursion-tree-figure>svg{display:block;width:100%;min-width:0;max-width:100%;
            height:min(60vh,560px);max-height:60vh;margin:0 auto;background:#fff;transform:translateZ(0);
          ...
          .tree-node.is-highlighted>circle,
          .tree-node.is-highlighted>.tree-node-equation,
          .tree-node.is-highlighted>.tree-node-math,
          .tree-edge.is-highlighted{opacity:1!important}
    ...
    def solve_builder(*_):
        ...
        if parsed is None or base_value is None:
            ...
            method_solution.layout.display = "none"
            return
        from simulaciones.capitulo5.runtime.recurrence_solution_methods import (
            SOLUTION_STYLES,
    ...
    display(Javascript(
        ...
        (() => {
          ...
          const pendingMathReveal = new Map();
          const rawMathPattern = /\\\(|\\\[/;
          const MAX_HIDDEN_MS = 2000;
          const hideUntypedMath = mutations => {
            mutations.forEach(mutation => {
              mutation.addedNodes.forEach(node => {
                ...
                if (!target || pendingMathReveal.has(target)) return;
                const text = target.textContent || '';
                if (rawMathPattern.test(text) && !target.querySelector('mjx-container')) {
                  target.style.visibility = 'hidden';
                  pendingMathReveal.set(target, Date.now());
                }
              });
            });
          };
          // Solo revela un nodo cuando confirmamos que ya tiene su <mjx-container>
          // (fórmula realmente convertida), o tras MAX_HIDDEN_MS como salvaguarda para
          // nunca dejar contenido oculto para siempre si algo impide el formateo.
          // Devuelve true si queda algún nodo pendiente (para reintentar el typeset).
          const reconcilePendingMath = () => {
            const now = Date.now();
            pendingMathReveal.forEach((hiddenAt, node) => {
              const typeset = node.querySelector('mjx-container');
              const expired = now - hiddenAt > MAX_HIDDEN_MS;
              if (typeset || expired) {
                node.style.visibility = '';
                pendingMathReveal.delete(node);
              }
            });
            return pendingMathReveal.size > 0;
          };
          ...
          const renderNodeMath = mathJax => Promise.all(
            Array.from(activeRoot.querySelectorAll('.tree-node-math:not([data-rendered])'))
              .map(async holder => {
                ...
                const width = enlarged ? 44 : 38;
                const height = enlarged ? 28 : 22;
          ...
          const typesetActiveApp = () => {
            ...
            typesetTimer = setTimeout(() => {
              ...
              window.__recursionTreeMathJaxReady
                ...
                .then(() => {
                  const stillPending = reconcilePendingMath();
                  applyZoom(activeRoot);
                  repaintGraph(activeRoot);
                  if (stillPending) typesetQueued = true;
                })
                .catch(error => {
                  reconcilePendingMath();
                  typesetQueued = true;
                  console.error(error);
                })
                .finally(() => {
