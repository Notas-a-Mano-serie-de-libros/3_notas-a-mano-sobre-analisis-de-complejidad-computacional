...
def _local_module():
    for base in (Path.cwd(), *Path.cwd().parents):
        candidate = base / "simulaciones" / "capitulo5" / "runtime" / "recursion_tree_animation.py"
        if candidate.exists():
...
def _download_common_runtime(module_dir, repository_root):
    ...
    _download(repository_root + "common/remote_fetch.py", common_dir / "remote_fetch.py")
    _download(repository_root + "common/simulation_views.py", common_dir / "simulation_views.py")
    domain_dir = module_dir / "simulaciones" / "capitulo5" / "runtime"
    domain_dir.mkdir(parents=True, exist_ok=True)
    (domain_dir.parent / "__init__.py").touch(exist_ok=True)
    (domain_dir / "__init__.py").touch(exist_ok=True)
    _download(
        repository_root + "simulaciones/capitulo5/runtime/recurrence_solution_methods.py",
...
def _activate_runtime(root):
    ...
    for module_name in tuple(sys.modules):
        if (
            module_name == "simulaciones"
            or module_name.startswith("simulaciones.")
...
def _load(path):
    name = "simulaciones.capitulo5.runtime.recursion_tree_animation"
...
if module_path is None:
    ...
    package_dir = module_dir / "simulaciones" / "capitulo5" / "runtime"
    package_dir.mkdir(parents=True, exist_ok=True)
    for directory in (package_dir, package_dir.parent, package_dir.parent.parent):
        (directory / "__init__.py").touch(exist_ok=True)
    module_path = package_dir / "recursion_tree_animation.py"
    base_url = (
        "https://raw.githubusercontent.com/Notas-a-Mano-serie-de-libros/"
        "3_notas-a-mano-sobre-analisis-de-complejidad-computacional/"
        "main/simulaciones/capitulo5/runtime/"
    )
    ...
    runtime_root = module_path.resolve().parents[3]
