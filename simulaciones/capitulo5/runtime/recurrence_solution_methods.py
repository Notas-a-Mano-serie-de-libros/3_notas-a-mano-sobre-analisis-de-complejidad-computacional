...
def _akra_root(coefficients, reductions):
    ...
    for _ in range(120):
        middle = (low + high) / 2
        value = sum(a * b**middle for a, b in zip(coefficients, reductions, strict=True))
        if value > 1:
            low = middle
        else:
            high = middle
...
def _master(
    ...
    if variant == "generalized":
        ...
        equation = "+".join(
            rf"{a:g}\cdot({b:g})^p" for a, b in zip(coefficients, reductions, strict=True)
        )
        general_solution = (
...
def _characteristic(
    ...
    polynomial[0] = 1.0
    for coefficient, lag in zip(coefficients, reductions, strict=True):
...
.method-solution-content mjx-container[display="true"]{font-size:16px!important;
