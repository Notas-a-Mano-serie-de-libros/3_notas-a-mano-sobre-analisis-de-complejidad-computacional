...
REQUIRED_FILES = (
    ...
    "simulaciones/capitulo7/runtime/launchers.py",
...
def resolve_launcher_path():
    for base in (Path.cwd(), *Path.cwd().parents):
        launcher_path = base / "simulaciones" / "capitulo7" / "runtime" / "launchers.py"
        if launcher_path.exists():
            project_root = str(launcher_path.parents[3].resolve())
            core_dir = str((launcher_path.parents[3] / "core" / "search").resolve())
            launcher_dir = str(launcher_path.parent.resolve())
            for path in (project_root, core_dir, launcher_dir):
                if path not in sys.path:
                    sys.path.insert(0, path)
            return launcher_path
    raise FileNotFoundError("No se pudo localizar simulaciones/capitulo7/runtime/launchers.py")


if running_in_colab():
    ensure_colab_files()
    # Usar los módulos completos descargados para este capítulo, aunque otra
    # celda haya cargado antes un runtime parcial de gráficas.
