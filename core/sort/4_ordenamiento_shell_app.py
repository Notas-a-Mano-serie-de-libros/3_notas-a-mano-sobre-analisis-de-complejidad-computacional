...
from sort_common import create_state as create_sort_state
    ...
    <div class="shell-comparison-bar-wrap">
      <div class="shell-comparison-bar-area">
        <div class="shell-comparison-bar-stack">
          ...
          <div class="shell-comparison-bar shell-comparison-bar-role-{role}" style="height:{height}px; background:{fill}; color:{text};"></div>
        </div>
      </div>
    </div>
    """
...
{SORT_THEME_CSS}
      ...
      .shell-comparison-bar-wrap + .shell-comparison-bar-wrap {{
        margin-left: -1px;
      }}
      .shell-comparison-bar-role-current,
      .shell-comparison-bar-role-pivot {{ border-color:var(--sort-border);box-shadow:none; }}
      .shell-comparison-bar-role-sorted::after {{ content:"✓";position:absolute;top:2px;right:2px;color:#2e7d32;font:700 10px/1 sans-serif; }}
      .shell-comparison-bar-role-excluded {{ opacity:var(--sort-muted-opacity); }}
...
def render_gap_comparison_rows_html(state):
    return "".join(
        render_cached_sequence_row(item, show_indexes=True)
        for item in state["algorithms"]
    )
...
def run_gap_comparison_app():
    ...
    style_output = widgets.HTML(
        value=render_gap_comparison_styles(),
