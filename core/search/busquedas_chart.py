...
from common.remote_fetch import read_text
...
_RAW_DOMAIN_URL = "https://raw.githubusercontent.com/Notas-a-Mano-serie-de-libros/3_notas-a-mano-sobre-analisis-de-complejidad-computacional/main/core/search"
...
def _ensure_search_metrics():
    ...
    if not metrics_path.exists():
        metrics_path.write_text(
            read_text(f"{_RAW_DOMAIN_URL}/search_metrics.py"),
            encoding="utf-8",
        )
...
def _simulate(algorithms: list, emp_n: list[int], trials: int) -> dict[str, list[float]]:
    ...
    for idx, n in enumerate(emp_n, 1):
        ...
        for _ in range(trials):
            ...
            for name, _mod, _step_fn, _kwargs, _ in algorithms:
