"""Infraestructura compartida de los laboratorios de ejercicios."""
...
from simulaciones.capitulo2.runtime.experimental_animation import formula_widget, mathjax_frame
from simulaciones.capitulo2.runtime.experimental_animation import run_app as _chapter2_run_app
...
.exercise-table table,.exercise-table thead,.exercise-table tbody,.exercise-table tr,.exercise-table th,.exercise-table td,.exercise-table span:not(.exercise-result-symbol):not(.exercise-loading),.exercise-table sup{color:#000!important}
.exercise-table th,.exercise-table td{box-sizing:border-box;height:42px;padding:6px 14px;text-align:center;white-space:nowrap}
.exercise-table th{font-family:sans-serif;font-weight:700;background:#fff!important;border-bottom:1px solid #9e9e9e}
...
.exercise-theory-status{font:14px sans-serif;color:#000!important}
.exercise-result-symbol{font-family:serif;font-size:28px;line-height:1;font-weight:700;color:#2d7d32!important}
.exercise-loading{display:inline-block;width:16px;height:16px;border:2px solid #bdc1c6;border-top-color:#1a73e8;border-radius:50%;animation:exercise-spin .75s linear infinite}
@keyframes exercise-spin{to{transform:rotate(360deg)}}
</style>
"""
...
def _chapter2_control_styles() -> str:
    ...
    return next(value for value in _chapter2_run_app.__code__.co_consts if isinstance(value, str) and ".constant-centered-input input" in value)
...
class ExerciseAlgorithm:
    ...
    input_builder: Callable[[int, str], tuple[Any, Any]]
    temporal: dict[str, tuple[str, Callable[[int], float]]]
    spatial: dict[str, tuple[str, Callable[[int], float]]]
    safe_n: int = PROJECTION_LIMIT
    temporal_measure: Callable[[Any, Any], float] | None = None
    spatial_measure: Callable[[Any, Any], float] | None = None
...
def _table_document(body: str, resource: str, unit: str) -> str:
    headings = (
        f"<thead><tr><th>Cantidad de datos (n)</th><th>{resource} teórico [{unit}]</th><th>{resource} experimental [{unit}]</th><th>Estado</th></tr></thead>"
    )
    return f'{_TABLE_STYLE}<div class="exercise-table"><table>{headings}<tbody>{body}</tbody></table></div>'


def _table(rows, unit):
    checkpoints = [row for row in rows if row["n"] >= 10 and 10 ** round(math.log10(row["n"])) == row["n"]]
    body = "".join(
    ...
    return _table_document(body, resource, unit)
...
def _pending_table(spec, analysis, maximum_n, case, states=None):
    ...
    for row in rows:
        ...
        if state == "loading":
            result = '<span class="exercise-equation">Ejecutando</span>'
            status = '<span class="exercise-loading" role="status" aria-label="Ejecutando"></span>'
        elif state == "complete":
            result = f'<span class="exercise-equation">{_number(value)}</span>'
            status = '<span class="exercise-result-symbol" role="img" aria-label="Completado">✓</span>'
        elif state == "projected":
            result = '<span class="exercise-equation">Proyección teórica</span>'
            status = '<span class="exercise-theory-status">Solo teórico</span>'
        else:
            result = '<span class="exercise-equation">Pendiente</span>'
            status = '<span class="exercise-theory-status">En espera</span>'
        rendered.append(
            f'<tr><td><span class="exercise-equation">10<sup>{int(math.log10(row["n"]))}</sup> = {row["n"]}</span></td><td><span class="exercise-equation">{_number(row["estimate"])}</span></td><td>{result}</td><td>{status}</td></tr>'
        )
    body = "".join(rendered)
...
def _figure_png_html(fig):
    ...
    return (
        '<div class="experimental-figure-frame" style="width:100%;max-width:800px;aspect-ratio:2/1;margin:0 auto;background:#fff;"><img alt="Gráfica del experimento" src="data:image/png;base64,'
        + encoded
        + '"></div>'
    )


def _configure_axis(ax, analysis: str) -> str:
...
def _plot_template_html(maximum_n, analysis):
    ...
    ax.set_xlim(0, maximum_n)
    ax.set_ylim(0, 1)
    _configure_axis(ax, analysis)
    fig.tight_layout()
    return _figure_png_html(fig)


def _plot_html(rows, analysis):
    plt.rcParams.update(_CHART_STYLE)
    fig, ax = plt.subplots(figsize=(8, 4.5), dpi=110)
    symbol = "T" if analysis == "temporal" else "S"
    ...
    if measured:
        ax.plot([r["n"] for r in measured], [r["value"] for r in measured], "-", linewidth=1.8, label=rf"${symbol}(n)$ experimental", color="#1f77b4")
    ax.plot([r["n"] for r in rows], theoretical, ":", linewidth=2.0, label=rf"${symbol}(n)$ teórica", color="#ff0000")
    _configure_axis(ax, analysis)
    fig.tight_layout()
    return _figure_png_html(fig)
...
def run_laboratory(algorithms: dict[str, ExerciseAlgorithm], chapter_title: str):
    ...
    maximum_value.layout = widgets.Layout(
        width="120px", min_width="120px", max_width="120px", height="32px", flex="0 0 120px", display="flex", align_items="center", justify_content="center"
    )
    ...
    controls_row = widgets.Box(
        [
            group("Búsqueda" if "7" in chapter_title else "Orden", algorithm),
            group("Análisis", analysis),
            group("Máximo n", maximum_stepper),
            group("Puntos de muestreo", sampling.container),
            group("Caso de ejecución", case),
        ],
        layout=widgets.Layout(
            width="auto", display="flex", flex_flow="column nowrap", grid_gap=f"{STANDARD_CONTROL_ROW_GAP}px", align_items="flex-start", overflow="visible"
        ),
    )
    ...
    figure_output = widgets.HTML(
        value='<div class="experimental-figure-frame" style="width:100%;max-width:800px;aspect-ratio:2/1"></div>',
    ...
    def run(_=None):
        ...
        try:
            ...
            rows, _ = experiment(spec, analysis.value, maximum_n(), sampling_points(), case.value, update_progress)
            table_output.value = _table(rows, "s" if analysis.value == "temporal" else "bytes")
            figure_output.value = _plot_html(rows, analysis.value)
        finally:
            button.disabled = False

    def restart(_=None):
        analysis.value = "temporal"
        case.value = "promedio"
        points_value.value = "1000"
        maximum_state["exponent"] = 5
        maximum_value.value = mathjax_frame(r"\(10^{5}\)", 30, centered=True)
        invalidate()
    ...
    css = widgets.HTML(
        value=_chapter2_control_styles() + standard_view_styles(".constant-animation-root"),
